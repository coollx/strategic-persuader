...
**Link & outcome:** https://openreview.net/forum?id=TWIlQMevJx — submitted to ICLR 2026 under the v1 title "GTAlign: Game-Theoretic Alignment of LLM Assistants for Mutual Welfare"; the OpenReview record lists it as `ICLR.cc/2026/Conference/Rejected_Submission` (rejected). Queried via the OpenReview API on 2026-08-11.
**Main criticisms / rebuttal:** the individual reviews could not be retrieved from this environment (both the forum page and the API's reply listing sit behind OpenReview's access-verification challenge), so no review content is summarized here — none of the above reflects reviewer opinion.
**Net takeaway:** treat this as a rejected-then-revised preprint: the v3 experimental story (welfare ablations, Pareto metrics, sensitivity analysis, user study) is unusually complete for a preprint, but no accepted-venue peer review vouches for it yet.
